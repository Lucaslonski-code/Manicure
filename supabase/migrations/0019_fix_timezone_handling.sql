-- ============================================================================
-- 0019_fix_timezone_handling.sql
-- ============================================================================
--
-- Fix timezone handling in RPCs to use explicit 'America/Sao_Paulo' timezone
-- instead of relying on session timezone. This prevents incorrect availability
-- checks and slot generation when the session timezone differs from the
-- business timezone.
--
...
AS $$
  ...
  slots AS (
    SELECT 
      generate_series(
        (p_date + ds.start_time)::TIMESTAMP AT TIME ZONE 'America/Sao_Paulo',
        (p_date + ds.end_time - INTERVAL '1 minute' * sd.duration)::TIMESTAMP AT TIME ZONE 'America/Sao_Paulo',
        INTERVAL '15 minutes'
      ) AS slot_start
...
BEGIN
  ...
  -- Validate time is within availability (using explicit timezone)
  IF NOT EXISTS (
    SELECT 1 FROM public.availability a
    WHERE a.professional_id = p_professional_id
      AND a.weekday = EXTRACT(DOW FROM (p_start_at AT TIME ZONE 'America/Sao_Paulo'))::INTEGER
      AND (p_start_at AT TIME ZONE 'America/Sao_Paulo')::TIME >= a.start_time
      AND (v_end_at AT TIME ZONE 'America/Sao_Paulo')::TIME <= a.end_time
  ) THEN
...
-- ============================================================================
-- Fix reschedule_appointment_by_admin
-- ============================================================================
...
BEGIN
  ...
  -- Validate within availability (using explicit timezone)
  IF NOT EXISTS (
    SELECT 1 FROM public.availability a
    WHERE a.professional_id = v_appointment_professional_id
      AND a.weekday = EXTRACT(DOW FROM (p_new_start_at AT TIME ZONE 'America/Sao_Paulo'))::INTEGER
      AND (p_new_start_at AT TIME ZONE 'America/Sao_Paulo')::TIME >= a.start_time
      AND (v_new_end_at AT TIME ZONE 'America/Sao_Paulo')::TIME <= a.end_time
  ) THEN
    RAISE EXCEPTION 'Time outside availability' USING ERRCODE = '42202';
  END IF;
